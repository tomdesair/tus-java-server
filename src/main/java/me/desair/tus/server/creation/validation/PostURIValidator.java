...
public class PostURIValidator extends AbstractRequestValidator {
    ...
    public void validate(final HttpMethod method, final HttpServletRequest request, final UploadStorageService uploadStorageService, final UploadIdFactory idFactory) throws TusException {
        if(!StringUtils.equals(request.getRequestURI(), idFactory.getUploadURI())) {
            throw new PostOnInvalidRequestURIException("POST requests have to be send to " + idFactory.getUploadURI());
        }
    }
